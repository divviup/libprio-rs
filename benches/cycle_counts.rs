...
fn idpf_poplar_gen(
    bits: usize,
    input: &IdpfInput,
    inner_values: Vec<[Field64; 2]>,
    leaf_value: [Field255; 2],
) {
    idpf::gen::<_, PrgAes128, 16, 2>(bits, input, inner_values, leaf_value).unwrap();
}
...
fn idpf_poplar_eval(
    ...
    idpf::eval::<PrgAes128, 16, 2>(bits, 0, public_share, key, input, &mut cache).unwrap();
}
...
fn idpf_poplar_eval_8() {
    ...
    idpf_poplar_eval(8, &input, &public_share, &key);
}
...
fn idpf_poplar_eval_128() {
    ...
    let key = Seed::get_decoded(&[0xff; 16]).unwrap();
    idpf_poplar_eval(128, &input, &public_share, &key);
}
...
fn idpf_poplar_eval_2048() {
    ...
    let key = Seed::get_decoded(&[0xff; 16]).unwrap();
    idpf_poplar_eval(2048, &input, &public_share, &key);
}
