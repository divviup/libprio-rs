...
macro_rules! make_field {
    (
        ...
        impl<'de> Deserialize<'de> for $elem {
            ...
            fn deserialize<D>(deserializer:D) -> Result<$elem, D::Error>
            where D: Deserializer<'de>,
            {
                struct FieldElementVisitor;
                impl<'de> de::Visitor<'de> for FieldElementVisitor {
                    type Value = $elem;

                    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                        write!(formatter, "a byte array of length $bytes")
                    }
                    fn visit_bytes<E>(self, bytes: &[u8]) -> Result<$elem, E>
                    where E: de::Error,
                    {
                        if $bytes > bytes.len() {
                            return Err(E::custom(FieldError::FromBytesShortRead));
                        }
                        ...
                        if int >= $fp.p {
                            return Err(E::custom(FieldError::FromBytesModulusOverflow));
                        }
                        Ok($elem($fp.elem(int)))
                    }
                }
