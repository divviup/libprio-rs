...
pub trait Word:
    ...
    + ConstOne
{
    /// Carry type allows the conversion of carry/borrow values
    /// produced by adding/subtracting to Word values.
...
pub trait FieldOps<W: Word>: FieldParameters<W> {
    ...
    fn sub(x: W, y: W) -> W {
        ...
        let mask = W::ZERO.wrapping_sub(&W::Carry::from(b0).as_());
        //      z0
        // +     p
        // ========
        //   s1,s0
...
pub(crate) trait FieldMulOpsSplitWord<W: Word>: FieldParameters<W> {
    /// Multiplication of field elements in the Montgomery domain.
    /// This uses the Montgomery's [REDC algorithm][montgomery].
    /// The result will be in [0, p).
    ///
