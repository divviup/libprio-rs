...
/// distributed point function. Variable bits denotes the maximum length of the
/// private label associated with any input.
#[derive(Clone, Debug)]
...
impl<'a, T, P, const SEED_SIZE: usize> ParameterizedDecode<(&'a Mastic<T, P, SEED_SIZE>, usize)> for MasticInputShare<T::Field, SEED_SIZE>
where
    T: Type,
    P: Xof<SEED_SIZE>,
{
    fn decode_with_param(
        ...
        let proofs_share = SzkProofShare::<T::Field, SEED_SIZE>::decode_with_param(decoding_parameter.0, bytes)?;
...
impl<'a, T, P, const SEED_SIZE: usize> ParameterizedDecode<(&'a Mastic<T, P, SEED_SIZE>, &'a MasticAggregationParam)> for MasticOutputShare<VidpfWeight<T::Field>>
where
    T: Type,
    P: Xof<SEED_SIZE> {
    fn decode_with_param(
    ...
    P: Xof<SEED_SIZE>,
{
    ...
    ) -> Result<(MasticPublicShare<VidpfWeight<T::Field>>, Vec<MasticInputShare<T::Field, SEED_SIZE>>), VdafError> {
